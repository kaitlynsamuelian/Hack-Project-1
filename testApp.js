...
function clearBoxes(){ [...previewWrap.querySelectorAll('.hitbox')].forEach(el=>el.remove()); }
...
// ===== OCR + wiring (K&E ids + better pipeline)
fileInput.addEventListener('change', async (e) => {
  ...
  try {
    const { data } = await Tesseract.recognize(canvas, 'eng', {
      logger: m => { if (m.status && typeof m.progress === 'number') progress.textContent = `${m.status} ${(m.progress*100|0)}%`; }
    });

    // Raw text for reference
    const rawLines = (data?.lines ?? []).map(l => l.text.trim()).filter(Boolean);
    const rawText  = rawLines.length ? rawLines.join('\n') : (data?.text ?? '').trim();
    ocrOut.textContent = rawText || '(no text detected)';
    ...
    // Render Parsed Dishes (your K&E list UI)
    if (!parsed.dishes.length) {
      ...
      showToast(`Found ${parsed.dishes.length} dishes (style: ${parsed.style})`);
    }
    ...
    if (debugOut) debugOut.textContent = dbg || 'No debug.';
  } catch (err) {
    console.error(err);
...
copyBtn.addEventListener('click', async () => {
  try { await navigator.clipboard.writeText(jsonOut.value); showToast('Copied JSON'); }
  catch { jsonOut.select(); document.execCommand('copy'); showToast('Copied (fallback)'); }
});
